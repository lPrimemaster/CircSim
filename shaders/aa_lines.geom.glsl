...
void fatlineFromLine(vec4 start, vec4 end)
{
	...
	gl_Position = (start + N * lineWidth / 2.0); //Start RHS
	EmitVertex();
	gl_Position = (start - N * lineWidth / 2.0); //Start LHS
	EmitVertex();

	gl_Position = (end + N * lineWidth / 2.0); //End RHS
	EmitVertex();
	gl_Position = (end - N * lineWidth / 2.0); //End LHS
	EmitVertex();
}
